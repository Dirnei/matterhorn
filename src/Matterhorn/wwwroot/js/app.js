...
document.addEventListener('keydown', e=>{ if (e.key==='Escape'){ closeMenu(true); closeUnpair(true); closeGroupMenu(true); closeGroupDelete(true); closeSceneMenu(true); closeSceneDelete(true); closeCapture(true); } });

async function loadAll(){
  let list; try { list = await (await api('/api/devices')).json(); } catch(e){ return; }
  defs.clear(); list.forEach(d=>defs.set(d.friendly_name,d));
  await Promise.all(list.map(async d=>{
    try { states.set(d.friendly_name, await (await api('/api/devices/'+encodeURIComponent(d.friendly_name))).json()); }
    catch { states.set(d.friendly_name,{}); }
  }));
  render();
  loadGroups();
  loadScenes();
...
function render(){
  ...
  const list=[...defs.values()].sort((a,b)=>a.friendly_name.localeCompare(b.friendly_name));
...
// ---- groups: cards with a master switch, member add/remove, kebab rename/delete ----
const groupsGrid = document.getElementById('groupsGrid');
const groupDefs = new Map();

async function loadGroups(){
  let list; try { list = await (await api('/api/groups')).json(); } catch(e){ return; }
  groupDefs.clear(); list.forEach(g=>groupDefs.set(g.friendly_name,g));
  renderGroups();
}

function renderGroups(){
  groupsGrid.innerHTML='';
  const list=[...groupDefs.values()].sort((a,b)=>a.friendly_name.localeCompare(b.friendly_name));
...
function groupCard(g){
  ...
  const available=[...defs.keys()].filter(n=>!members.includes(n));
  el.innerHTML =
    `<div class="st-head">
  ...
  el.querySelectorAll('.chip button').forEach(b=>b.addEventListener('click', async ()=>{
    ...
    try {
      ...
      if(!r.ok) toast('Remove failed'); else loadGroups();
  ...
  sel.addEventListener('change', async ()=>{
    ...
    try {
      ...
      if(!r.ok) toast('Add failed'); else loadGroups();
...
async function doDeleteGroup(name){
  try {
    ...
    else { toast('Deleting…'); loadGroups(); }
  } catch(e){ if(e.message!=='401') toast('Delete failed'); }
}

// ---- scenes: chips/cards with a Recall action, kebab rename/delete, and a capture-scene modal ----
...
async function loadScenes(){
  ...
  sceneDefs.clear(); list.forEach(s=>sceneDefs.set(s.friendly_name,s));
  renderScenes();
}

function renderScenes(){
  scenesGrid.innerHTML='';
  const list=[...sceneDefs.values()].sort((a,b)=>a.friendly_name.localeCompare(b.friendly_name));
...
async function doDeleteScene(name){
  try {
    ...
    else { toast('Deleting…'); loadScenes(); }
  } catch(e){ if(e.message!=='401') toast('Delete failed'); }
}
...
function openCapture(){
  ...
  const list=[...defs.keys()].sort((a,b)=>a.localeCompare(b));
...
document.getElementById('captureForm').addEventListener('submit', async ev=>{
  ...
  try {
    ...
    else { toast('Scene captured'); closeCapture(); loadScenes(); }
...
document.getElementById('newGroupForm').addEventListener('submit', async ev=>{
  ...
  try {
    ...
    else { toast('Group created'); input.value=''; loadGroups(); }
