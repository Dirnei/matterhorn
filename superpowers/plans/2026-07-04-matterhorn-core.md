# Matterhorn Core — Implementation Plan
...
- **Base topic** default `matterhorn`; **REST port** default `8090`; **API key** required for REST mutations (spec §8, §10).
...
- New repo `matterhorn`, separate from `vidar`. All paths below are relative to the `matterhorn` repo root.
...
Matterhorn.sln
src/Matterhorn/
  Matterhorn.csproj                 (Microsoft.NET.Sdk.Web)
  ...
  Config/
    MatterhornConfig.cs
tests/Matterhorn.Tests/
  Matterhorn.Tests.csproj
...
**Files:**
- Create: `Matterhorn.sln`, `src/Matterhorn/Matterhorn.csproj`, `tests/Matterhorn.Tests/Matterhorn.Tests.csproj`
- Create: `src/Matterhorn/Domain/MatterClusters.cs`
- Test: `tests/Matterhorn.Tests/Domain/SmokeTests.cs`
...
```bash
dotnet new sln -n Matterhorn
dotnet new web -n Matterhorn -o src/Matterhorn
dotnet new xunit -n Matterhorn.Tests -o tests/Matterhorn.Tests
dotnet sln add src/Matterhorn/Matterhorn.csproj tests/Matterhorn.Tests/Matterhorn.Tests.csproj
dotnet add tests/Matterhorn.Tests reference src/Matterhorn
```

- [ ] **Step 2: Add packages**

```bash
dotnet add src/Matterhorn package Akka.Hosting
dotnet add src/Matterhorn package Akka.Streams
dotnet add src/Matterhorn package HiveMQtt
dotnet add tests/Matterhorn.Tests package Akka.TestKit.Xunit2
dotnet add tests/Matterhorn.Tests package Akka.Streams.TestKit
```
...
```csharp
using Matterhorn.Domain;
using Xunit;

namespace Matterhorn.Tests.Domain;
...
```bash
git add -A
git commit -m "chore: scaffold Matterhorn solution and cluster constants"
```
...
**Files:**
- Create: `src/Matterhorn/Domain/AttributeReading.cs`, `src/Matterhorn/Domain/PropertyMapping.cs`
- Test: `tests/Matterhorn.Tests/Domain/PropertyMappingTests.cs`
...
**Files:**
- Create: `src/Matterhorn/Domain/CommandSpec.cs`, `src/Matterhorn/Domain/CommandMapping.cs`
- Test: `tests/Matterhorn.Tests/Domain/CommandMappingTests.cs`
...
**Files:**
- Create: `src/Matterhorn/Domain/FriendlyName.cs`, `src/Matterhorn/Domain/DeviceDescriptor.cs`, `src/Matterhorn/Domain/ExposesBuilder.cs`
- Test: `tests/Matterhorn.Tests/Domain/FriendlyNameTests.cs`, `tests/Matterhorn.Tests/Domain/ExposesBuilderTests.cs`
...
**Files:**
- Create: `src/Matterhorn/Controller/MatterEvents.cs`, `src/Matterhorn/Controller/IMatterController.cs`, `src/Matterhorn/Controller/FakeMatterController.cs`
- Test: `tests/Matterhorn.Tests/Controller/FakeMatterControllerTests.cs`
...
```csharp
using System.Text.Json;
using Matterhorn.Controller;
using Matterhorn.Domain;
using Xunit;

namespace Matterhorn.Tests.Controller;
...
// MatterEvents.cs
using Matterhorn.Domain;
namespace Matterhorn.Controller;
...
**Files:**
- Create: `src/Matterhorn/Mqtt/MqttTopics.cs`, `src/Matterhorn/Mqtt/IMqttPublisher.cs`, `src/Matterhorn/Mqtt/HiveMqttPublisher.cs`
- Test: `tests/Matterhorn.Tests/Mqtt/MqttTopicsTests.cs`
...
namespace Matterhorn.Tests.Mqtt;
...
```csharp
// tests/Matterhorn.Tests/Mqtt/InMemoryMqttPublisher.cs
...
**Files:**
- Create: `src/Matterhorn/Actors/ActorMessages.cs`, `src/Matterhorn/Actors/MatterEndpointActor.cs`
- Test: `tests/Matterhorn.Tests/Actors/MatterEndpointActorTests.cs`
...
using Matterhorn.Actors;
using Matterhorn.Controller;
using Matterhorn.Domain;
using Matterhorn.Mqtt;
using Matterhorn.Tests.Mqtt;
using Xunit;

namespace Matterhorn.Tests.Actors;
...
public class MatterEndpointActorTests : TestKit
{
    ...
    public void Merges_multiple_attributes_into_one_state()
    {
        ...
        AwaitAssert(() =>
        {
            var last = mqtt.Messages.Where(m => m.Topic == "matterhorn/lamp").OrderBy(_ => 0).Last();
...
**Files:**
- Create: `src/Matterhorn/Streaming/IngestionPipeline.cs`
- Test: `tests/Matterhorn.Tests/Streaming/IngestionPipelineTests.cs`
...
using Matterhorn.Controller;
using Matterhorn.Domain;
using Matterhorn.Streaming;
using Xunit;

namespace Matterhorn.Tests.Streaming;
...
using Matterhorn.Actors;
using Matterhorn.Controller;
namespace Matterhorn.Streaming;
...
**Files:**
- Create: `src/Matterhorn/Actors/MatterGatewayActor.cs` (extend `ActorMessages.cs`)
- Test: `tests/Matterhorn.Tests/Actors/MatterGatewayActorTests.cs`
...
**Files:**
- Create: `src/Matterhorn/Rest/ApiKeyMiddleware.cs`, `src/Matterhorn/Rest/ApiEndpoints.cs`, `src/Matterhorn/Config/MatterhornConfig.cs`
- Test: `tests/Matterhorn.Tests/Rest/ApiEndpointsTests.cs`

**Interfaces:**
- Consumes: `MatterGatewayActor` messages (`GetDevices`, `SetDevice`, `CommissionRequest`).
- Produces:
  - `record MatterhornConfig(string ControllerWsUrl, string ControllerKind, string MqttHost, int MqttPort, string? MqttUser, string? MqttPassword, string BaseTopic, bool RestEnabled, int RestPort, string? ApiKey, string? ThreadDataset)` + `static MatterhornConfig FromConfiguration(IConfiguration)`.
...
namespace Matterhorn.Tests.Rest;
...
public static class ApiEndpoints
{
    public static void Map(WebApplication app)
    {
        ...
        app.MapGet("/api/bridge/info", () => Results.Json(new { service = "matterhorn" }));
    }
}
...
using HiveMQtt.Client;
using Matterhorn.Actors;
using Matterhorn.Config;
using Matterhorn.Controller;
using Matterhorn.Mqtt;
using Matterhorn.Rest;
...
builder.Services.AddAkka("matterhorn", b => b
...
# Expect: log "online" published to matterhorn/bridge/state; GET http://localhost:8090/api/bridge/info returns JSON.
```
...
**Files:**
- Create: `src/Matterhorn/Controller/MatterServerProtocol.cs`, `src/Matterhorn/Controller/PythonMatterServerController.cs`
- Test: `tests/Matterhorn.Tests/Controller/MatterServerProtocolTests.cs`
...
Document (spec §11): run a matter.js example on-off/dimmable device, commission it over IP via the running Matter server, point Matterhorn at that server (`Controller__Kind=python-matter-server`, `Controller__WsUrl=...`), and verify:
- `matterhorn/bridge/devices` lists the bulb with `state`+`brightness` exposes;
- toggling `state` via `matterhorn/<name>/set` `{"state":"ON"}` changes the device;
...
- §10 config keys → Task 10 (`MatterhornConfig`). ✅
