# Matterhorn — Design Spec

**Date:** 2026-07-04
**Status:** Approved for planning
**Repo:** new standalone repo (`matterhorn`), separate from `vidar`
...
### Guiding principle
Do one thing and expose it so others build on it. Matterhorn makes Matter device
...
- **No Thread/BLE commissioning logic in this service.** Physical commissioning
  (BLE radio placement, Thread dataset) is upstream/operational. Matterhorn only
...
```
┌───────────────────────────┐        ┌──────────────────────────┐   WS   ┌──────────────────────────────┐
│ SLZB-MR5U (192.168.0.233)  │        │  matterjs-server          │◄──────►│ Matterhorn (.NET / Akka.NET) │──► MQTT
...
- **Matterhorn** — this project. Pure .NET/Akka.NET. Consumes the controller WS,
...
| Topic | Dir | Payload |
|---|---|---|
| `matterhorn/bridge/state` | pub | `{"state":"online\|offline"}` (LWT) |
| `matterhorn/bridge/info` | pub | `{version, matterjs_version, fabric_id, commissioning_open, log_level}` |
| `matterhorn/bridge/devices` | pub | retained JSON array — discovery contract (§7) |
| `matterhorn/bridge/event` | pub | `{type, data}`: `device_joined`, `device_leave`, `device_interview`, `commission_started/completed/failed` |
| `matterhorn/bridge/request/<action>` | sub | management + commissioning (§6) |
| `matterhorn/bridge/response/<action>` | pub | result, correlated by `transaction` |

### Per-device topics

| Topic | Dir | Payload |
|---|---|---|
| `matterhorn/<friendly_name>` | pub | retained state, e.g. `{"state":"ON","brightness":254,"color_temp":370}` |
| `matterhorn/<friendly_name>/set` | sub | `{"state":"ON","brightness":128}` — keys → Matter commands |
| `matterhorn/<friendly_name>/set/<attr>` | sub | single-attr shortcut, e.g. `.../set/state` = `ON` |
| `matterhorn/<friendly_name>/get` | sub | `{"state":""}` forces a read |
| `matterhorn/<friendly_name>/availability` | pub | `online`/`offline` from subscription/reachability |
...
## 6. Commissioning (Matterhorn owns pairing — the "permit_join" analogue)
...
- **`MatterEndpointActor`** (one per logical device) — holds endpoint state; maps
  clusters → properties; publishes retained `matterhorn/<name>` + `/availability`;
...
| `mqtt.baseTopic` | base topic | `matterhorn` |
...
- Matterhorn (actors, mapping, MQTT, REST) is developed against this fake device.
...
- **Phase 1 — Matterhorn core (this spec).** Controller WS → actors → MQTT (+ REST).
...
- **Phase 2 — Vidar Matter plugin.** A `PluginActorBase` clone of the Zigbee2Mqtt
  plugin that consumes the Matterhorn MQTT topics. Small.
...
- **Multi-fabric** — devices already paired to Apple/Google. Out of scope for Phase 1;
  Matterhorn owns its own fabric.
```
